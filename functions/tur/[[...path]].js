export async function onRequest(context) {
  ...
  // /tur/pool/... → GitHub Release tag=0.1，缓存 1 天
  ...
  // 其他情况 → tur-mirror.pages.dev，不缓存
  const upstreamPath = path.replace("/tur", "");
  const upstream = `https://tur-mirror.pages.dev${upstreamPath}`;
  const resp = await fetch(upstream, { headers: context.request.headers });
  const newHeaders = new Headers(resp.headers);
  newHeaders.set("Cache-Control", "no-store");
  return new Response(resp.body, { status: resp.status, headers: newHeaders });
}
